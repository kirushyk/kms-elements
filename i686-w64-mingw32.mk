...
$(TARGET_DIR)/$(KMSHTTPEP_TARGET): $(KMSHTTPEP_C_OBJS) $(KMSHTTPEP_CXX_OBJS)
	mkdir -p $(TARGET_DIR)
	$(AR) cr $(TARGET_DIR)/$(KMSHTTPEP_TARGET) $^
	$(RANLIB) $(TARGET_DIR)/$(KMSHTTPEP_TARGET)

$(TARGET_DIR)/$(KMSELEMENTSINTERFACE_TARGET): $(KMSELEMENTSINTERFACE_OBJS)
	mkdir -p $(TARGET_DIR)
	$(AR) cr $(TARGET_DIR)/$(KMSELEMENTSINTERFACE_TARGET) $^
	$(RANLIB) $(TARGET_DIR)/$(KMSELEMENTSINTERFACE_TARGET)

$(TARGET_DIR)/$(RTPENDPOINT_TARGET): $(RTPENDPOINT_OBJS)
	mkdir -p $(TARGET_DIR)
	$(CC) -shared -o $(TARGET_DIR)/$(RTPENDPOINT_TARGET) $(CFLAGS) $^ $(RTPENDPOINT_LIBS) -Wl,--out-implib,$(TARGET_DIR)/$(RTPENDPOINT_TARGET).a

$(TARGET_DIR)/$(WEBRTCDATAPROTO_TARGET): $(WEBRTCDATAPROTO_OBJS)
	mkdir -p $(TARGET_DIR)
	$(CC) -shared -o $(TARGET_DIR)/$(WEBRTCDATAPROTO_TARGET) $(CFLAGS) $^ $(WEBRTCDATAPROTO_LIBS) -Wl,--out-implib,$(TARGET_DIR)/$(WEBRTCDATAPROTO_TARGET).a

$(TARGET_DIR)/$(KMSWEBRTCENDPOINTLIB_TARGET): $(KMSWEBRTCENDPOINTLIB_OBJS)
	mkdir -p $(TARGET_DIR)
	$(CC) -shared -o $(TARGET_DIR)/$(KMSWEBRTCENDPOINTLIB_TARGET) $(CFLAGS) $^ $(KMSWEBRTCENDPOINTLIB_LIBS) -Wl,--out-implib,$(TARGET_DIR)/$(KMSWEBRTCENDPOINTLIB_TARGET).a

$(TARGET_DIR)/$(KMSELEMENTSIMPL_TARGET): $(KMSELEMENTSIMPL_OBJS)
	mkdir -p $(TARGET_DIR)
	$(CXX) -shared -o $(TARGET_DIR)/$(KMSELEMENTSIMPL_TARGET) $(CFLAGS) $^ $(KMSELEMENTSIMPL_LIBS) -Wl,--out-implib,$(TARGET_DIR)/$(KMSELEMENTSIMPL_TARGET).a

$(TARGET_DIR)/$(KMSELEMENTSMODULE_TARGET): $(KMSELEMENTSMODULE_OBJS)
	mkdir -p $(TARGET_DIR)
	$(CXX) -shared -o $(TARGET_DIR)/$(KMSELEMENTSMODULE_TARGET) $(CFLAGS) $^ $(KMSELEMENTSMODULE_LIBS) -Wl,--out-implib,$(TARGET_DIR)/$(KMSELEMENTSMODULE_TARGET).a
